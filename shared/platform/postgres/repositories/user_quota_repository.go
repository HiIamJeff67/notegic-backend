...
type UserQuotaRepositoryInterface interface {
	GetRoutineTaskCostUnitUsed(ctx context.Context, userId uuid.UUID, opts ...RepositoryOptions) (int64, *cexceptions.Exception)
	InitializeMissing(ctx context.Context, now time.Time, opts ...RepositoryOptions) *cexceptions.Exception
	InitializeMissingForUserIds(ctx context.Context, userIds []uuid.UUID, now time.Time, opts ...RepositoryOptions) *cexceptions.Exception
	ResetDue(ctx context.Context, now time.Time, opts ...RepositoryOptions) (int64, *cexceptions.Exception)
	ConsumeRoutineTaskCostUnits(ctx context.Context, consumptionInputs []inputs.ConsumeRoutineTaskCostUnitInput, opts ...RepositoryOptions) ([]uuid.UUID, *cexceptions.Exception)
}

type UserQuotaRepository struct {
	db         *gorm.DB
	exceptions exceptions.UserQuotaException
}

func NewUserQuotaRepository(db *gorm.DB) UserQuotaRepositoryInterface {
	return &UserQuotaRepository{
		db: db, exceptions: exceptions.NewUserQuotaException()}
}
...
func (r *UserQuotaRepository) GetRoutineTaskCostUnitUsed(
	...
	if userId == uuid.Nil {
		return 0, r.exceptions.NewForDomain(
			"InvalidDto",
			"UserQuota",
...
func (r *UserQuotaRepository) initializeMissing(
	...
	if result := parsedOptions.DB.
		...
		Where("status = ?", cenums.UsersToBillingPlansStatus_Active).
