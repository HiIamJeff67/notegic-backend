...
func (UsersToStations) TableName() string {
	return postgres.TableName_UsersToStationsTable.String()
}

// UsersToStations Table Relations
type UsersToStationsRelation postgres.RelationName
...
func (uts *UsersToStations) AfterSave(tx *gorm.DB) error {
	if uts.Permission != cenums.AccessControlPermission_Owner {
		return nil
	}
