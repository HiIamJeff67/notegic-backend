...
type RoutineRecord struct {
	...
	RoutineId        uuid.UUID                  `json:"routineId" gorm:"column:routine_id; type:uuid; not null; uniqueIndex:routine_record_idx_routine_id_scheduled_at;"`
	Status           cenums.RoutineRecordStatus `json:"status" gorm:"column:status; type:\"RoutineRecordStatus\"; not null; default:'Pending';"`
	ScheduledAt      time.Time                  `json:"scheduledAt" gorm:"column:scheduled_at; type:timestamptz; not null; uniqueIndex:routine_record_idx_routine_id_scheduled_at;"`
