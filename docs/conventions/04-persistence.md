...
- Every repository ORM query must begin with `parsedOptions.DB.Model(&schemas.Xxx{})`, using the schema that owns the operation. Do not use `db.Table(...)` or `parsedOptions.DB.Table(...)`: it bypasses schema metadata and can produce incorrect model/table behavior. Raw SQL may name its required quoted tables explicitly, but its GORM session must still start from the owning `Model(&schemas.Xxx{})`.
- Name parsed repository options `parsedOptions`, never a shortened `parsed`. Treat options parsing as preparation: leave one blank line before the query/result block. Keep a query result and its immediate error handling adjacent; use one blank line only when moving to a separate preparation, query, mapping, or return stage.
...
- Use create/update/partial-update types from `shared/platform/postgres/repositories/inputs`. Do not pass a request directly to GORM. All repository inputs are shared persistence contracts; runtime-specific workflow DTOs remain in the runtime service layer.
...
1. Define `UpdateXxxInput` under `shared/platform/postgres/repositories/inputs/<domain>_input.go`. Use pointers to distinguish values supplied by this request and keep the correct `json` and `gorm:"column:..."` tags.
...
- Runtime-owned exception builders belong under each runtime's `runtimes/<runtime>/exceptions/`. PostgreSQL repository exceptions are split by domain under `shared/platform/postgres/repositories/exceptions/`, injected into each repository through its `exceptions` field, and used as `r.exceptions...`. All PostgreSQL GORM table schemas, relations, table names, repository implementations, repository inputs, and repository exceptions belong under `shared/platform/postgres/`. Runtime-owned views, triggers, and constraints belong under the owning runtime's `data/postgres/` directory. Runtimes import shared persistence contracts directly and must not duplicate them. Every repository constructor receives the owning runtime's DB pool; shared PostgreSQL code must not keep a package-global default DB or import a runtime. Runtime-specific business methods stay in services/workers. Runtime-owned enums remain under `runtimes/<service>/data/postgres/schemas/enums/` because their PostgreSQL scan/value behavior is runtime persistence code. Contract enum values remain under `contracts/types/enums/`.
- Every PostgreSQL runtime opens its own pool in the application composition root with `spostgres.Connect` and closes it during shutdown. Migration belongs to an explicit deployment/admin command, not application startup or `Connect`.
- The default pool is runtime-owned if a runtime needs one: `shared/platform/postgres/repositories` must not keep a package-global DB or `SetDefaultDB`; each runtime composition root constructs shared repository instances with its own pool, while `WithDB` and `WithTransactionDB` override it for request and transaction scope.
- All reusable PostgreSQL scopes belong under `shared/platform/postgres/scopes/`; scopes are not runtime-owned. Migration DDL belongs to the runtime that owns the migrated tables under that runtime's `data/postgres/views/`, `data/postgres/triggers/`, or `data/postgres/constraints/` directory. Runtime-only raw query helpers belong under that runtime's `data/postgres/sqls/`; they are not shared platform assets.
- Each database-owning runtime declares one `migration_manifest.go` under `runtimes/<service>/data/postgres/`. Its `MigrationManifest.Runtime` uses `contracts/types.Runtime` and contains the inline enum map and shared schema/DDL registrations handled by that runtime. A deployment runs the existing `bootstrapDB`, `migrateDB`, and (for Core) `seedDB` Cobra commands in order through a one-shot `*-database-init` service; no wrapper `initDB` command is required. The commands bootstrap the required role, call `postgres.Migrate()` on a pinned admin connection, apply `DatabasePermissionManifest`, and call `postgres.VerifyPermissions`. Each migration phase uses one transaction to grant the owner temporary schema `CREATE`, execute DDL with `SET ROLE`, reset the role, and revoke `CREATE`; failure or interruption rolls back the grant with the phase. Verification is read-only and checks non-owner runtime grants, `PUBLIC` ACLs, and the owner's schema default ACLs; its error must name the drifted runtime/privilege/object. Compose passes admin credentials only to these one-shot jobs; application startup must not receive them, bootstrap roles, migrate, or reconcile permissions.
- Seed data follows the same ownership boundary as migration. `shared/platform/postgres` defines `SeedManifest` and `Seed`; the owning runtime declares its `SeedManifest` under `runtimes/<service>/data/postgres/`. A seed job connects with the admin configuration, executes the manifest inside one transaction with `SET LOCAL ROLE` for the manifest runtime, and therefore relies on that runtime's normal table privileges. Seeds do not have a separate permission manifest; the owner runtime's migration permission setup is the authorization boundary. Compose runs the seed job only after the owner's migration and permission application succeed.
