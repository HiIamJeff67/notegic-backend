...
- Every PostgreSQL runtime must expose the same lifecycle in `data/postgres/database.go`: `DB *gorm.DB`, `Connect(spostgres.Config) (*gorm.DB, error)`, and `Disconnect(*gorm.DB) error`. `Connect` only opens and stores the runtime's pool; migration belongs to the application or database command. `Disconnect` closes the pool and clears the runtime default. Do not add runtime-specific database registries or move migration into `Connect`.
- The default pool is runtime-owned: each runtime's `data/postgres/database.go` may keep its own unexported `defaultDB` and expose it only to that runtime's composition layer. `shared/platform/postgres/repositories` must not keep a package-global DB or `SetDefaultDB`; each runtime composition root constructs shared repository instances with its own pool, while `WithDB` and `WithTransactionDB` override it for request and transaction scope.
...
- Each database-owning runtime declares one `migration_manifest.go` under `internal/<service>/data/postgres/`. Its `MigrationManifest.Runtime` uses `contracts/types.Runtime` and contains the inline enum map and shared schema/DDL registrations handled by that runtime. Use `postgres.Migrate()` so the requested `Runtime_*` value is checked before migration. Database permissions are a separate deployment phase.
