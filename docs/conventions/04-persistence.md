...
- Use create/update/partial-update types from the owning runtime's PostgreSQL `repositories` package. Do not pass a request directly to GORM.
...
1. Define `UpdateXxxInput` under `data/postgres/repositories/inputs/<domain>_input.go`; use pointers to distinguish values supplied by this request and keep the correct `json` and `gorm:"column:..."` tags.
...
- All PostgreSQL GORM table schemas, relations, table names, and shared DDL packages belong under `shared/platform/postgres/`. Runtimes import those schemas directly and must not keep duplicate local table models. Runtime-specific repositories belong under `internal/<service>/data/postgres/repositories/`, while their input types belong under the sibling subpackage `internal/<service>/data/postgres/repositories/inputs/` and are imported as `inputs`. Runtime-owned enums remain under `internal/<service>/data/postgres/schemas/enums/` because their PostgreSQL scan/value behavior is runtime persistence code. Contract enum values remain under `contracts/types/enums/`.
- Every PostgreSQL runtime must expose the same lifecycle in `data/postgres/database.go`: `DB *gorm.DB`, `Connect(platformpostgres.Config) (*gorm.DB, error)`, and `Disconnect(*gorm.DB) error`. `Connect` only opens and stores the runtime's pool; migration belongs to the application or database command. `Disconnect` closes the pool and clears the runtime default. Do not add runtime-specific database registries or move migration into `Connect`.
- A scope belongs under `shared/platform/postgres/scopes/` when it is a reusable persistence query fragment used by two or more runtimes. Keep scopes that encode one runtime's private workflow beside that runtime's repositories. SQL files follow the same rule: reusable SQL that only depends on shared PostgreSQL schemas belongs under `shared/platform/postgres/sqls/`; authentication, private projections, and other runtime-owned SQL stays under that runtime's `data/postgres/sqls/`.
